# M4 Answers for the B1–B4 Architecture Review

> **Status:** M4 engineering response and implementation update. B1, B2, and B3 are architecture drafts or software references; the GUI must not present any draft decision as a completed hardware security capability.

## Implementation update after patent-scope review

The repository contains a simulation-only `TrustedController` model under `security/trusted_controller.py`. It models separate `known_attack` and `adaptive_anomaly` evidence, HMAC authentication, payload hashing, freshness checks, replay/out-of-order rejection, controller states, optional quorum configuration, and authenticated monotonic containment receipts. The controller is deliberately labeled as a simulation scaffold; physical enforcement must be implemented in the ESP32 or dedicated security MCU.

The compact 480×320 M4 dashboard in `gui/dashboard.py` displays controller state, relay acknowledgment, tamper state, key state, primary-power status, independent-signal count, receipt verification, quorum configuration, and recovery status. The hardware simulator state API exposes matching telemetry fields for controller, signals, quorum, receipt, power, and recovery.
...
1. A message was authenticated by the controller rather than merely received through serial, UDP, or ESP-NOW.
...
M4 must not label a received alert as trusted merely because it arrived through a transport.

### B2 — Crypto shortlist

B2 leans toward HMAC-SHA256 because it is simpler and likely cheaper for candidate ESP32 targets, but it also records an important counterargument for ECDSA: per-node key isolation. B2 says the actual chip family, crypto toolchain, key provisioning, rotation, and compromise model are not yet finalized.
...
If quorum is not configured or verified, M4 must show `QUORUM: NOT CONFIGURED` rather than fabricated votes.

### B4 — Gap analysis and final visibility

B4 identifies final GUI visibility and recovery as an M4-owned feature, with M1, M2, and M3 supporting it. M4 coverage must show the real security state rather than only process supervision or simulated values.

The final dashboard should visibly cover these independent state categories:
...
3. Two independent valid signals: the GUI shows the separate evidence and final controller decision.
...
10. Recovery: wrong authentication leaves the controller locked; successful authenticated recovery displays controller acknowledgment before showing restoration.

## 7. Inputs required from other modules

M4 needs the following contracts from the other members:
...
- From the repository owner: branch and merge status.

## 8. Conclusion

M4’s immediate responsibility is documentation and visibility, not implementation of B1–B4 cryptography or quorum logic. The correct deliverable is this `M4_Answers_For_Meeting.md` file, together with GUI validation against normalized integration events. The designated repository owner remains responsible for merging the integration branch into `main` without force-pushing.
